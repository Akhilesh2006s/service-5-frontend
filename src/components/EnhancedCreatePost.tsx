...
export const EnhancedCreatePost: React.FC<EnhancedCreatePostProps> = ({ user, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        const uploadResult = await mediaService.uploadFiles(selectedFiles);
        if (uploadResult.success) {
          mediaFiles = uploadResult.mediaFiles;
        } else {
          // Fallback to local URLs if upload fails
          mediaFiles = selectedFiles.map(file => mediaService.createLocalMediaFile(file));
        }
      }
      ...
      <Card className="max-w-2xl mx-auto border-0 shadow-2xl bg-white/95 backdrop-blur-sm">
        ...
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-semibold text-gray-700">Add Media (Optional)</Label>
              <p className="text-xs text-gray-500">
                Upload up to 5 images and 1 video (max 1 minute) to provide visual evidence
              </p>
              <FileUpload
